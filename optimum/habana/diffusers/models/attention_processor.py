...
class GaudiQwenDoubleStreamAttnProcessor2_0:
    ...
    def __call__(
        ...
        if self.cp_size > 1:
            bs, txt_kv_seq, num_head, head_dim = txt_key.shape
            txt_key = txt_key.reshape(bs, txt_kv_seq, -1)
            txt_value = txt_value.reshape(bs, txt_kv_seq, -1)

            txt_full_key = torch.empty(
                bs, txt_kv_seq * self.cp_size, num_head * head_dim, dtype=txt_key.dtype, device=txt_key.device
            )
            txt_full_value = torch.empty(
                bs, txt_kv_seq * self.cp_size, num_head * head_dim, dtype=txt_value.dtype, device=txt_value.device
            )
